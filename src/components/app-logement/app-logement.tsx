...
export class AppLogement {
  @Prop() logements: { nom: string; email: string; rue: string; code_postal: string; ville: string; descriptif: string };
  @Prop() Logement: any;

  async componentWillRender() {
    const { data: logements } = await supabase.from('Logement').select('nom,email,rue,code_postal,ville,descriptif').gt('idLogement', 1);
    this.Logement = JSON.parse(JSON.stringify(logements));
          ...
          <div class="grid grid-cols-5 gap-4">
            {this.Logement.map((item: any = {}) => (
              <div class="bg-white p-6 rounded-lg shadow mx-3 my-7 border border-or">
                ...
                {item.rue} {item.code_postal}<br></br>
                {item.ville}
